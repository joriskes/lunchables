<script lang="ts">
  ...
  onMount(() => {
    ...
    nestedClick.addEventListener('click', (ev) => {
  ...
  function highlight(el: HTMLDivElement, mode: 'capture' | 'bubble', extra: 'stop' | '' = ''): void {
    setTimeout(() => {
      if (extra) {
...
<div class="nested-click" bind:this={nestedClick}>
  {#if n < 6}
    <svelte:self top={false} n={n+1} on:increment={increment} bubblestopat={bubblestopat} capturestopat={capturestopat}></svelte:self>
  {:else}
    &nbsp;
  {/if}
</div>
...
<style>
  ...
  .nested-click {
    ...
    margin-top: 3cqb;
    padding: .7cqb .3cqi .3rem .5rem;
    min-height: clamp(23%, clamp(4em, 4rem, 100cqb), 100cqb); /* clamp(40%, clamp(4em, *//* 10dvh *//* 10rem, 100cqb), 100cqb) */;
    min-width: 20cqi;
