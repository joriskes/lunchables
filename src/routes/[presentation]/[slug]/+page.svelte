...
<div class="flex flex-col items-center justify-between w-screen h-screen max-h-screen slide">
	...
	{:else if data.meta.type === 'demo'}
		<section class="w-5/6 h-full flex flex-col pt-10 mx-auto">
			<hgroup class="flex items-center w-full mb-10">
				<h1 class="w-full text-2xl font-bold text-center">{data.meta.title}</h1>
			</hgroup>

			<h1 class="font-black text-center text-yellow-400 text-7xl has-subtitle">{data.meta.subtitle}</h1>

			<div class="flex flex-col flex-grow items-center justify-center gap-4 p-4 overflow-auto border border-gray-800 overflow rounded-2xl max-h-[80vh]">
				<svelte:component this={data.content} />
			</div>
		</section>
