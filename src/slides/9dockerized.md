...
<div class="flex gap-8 h-full text-sm">
	<div class="flex-1 flex flex-col gap-4 text-gray-200 pt-1">
		...
		<p>This matters for agentic mode — the LLM needs to <em>run</em> commands and <em>read their output</em> to verify its own work. Without it, commands hang or fail silently.</p>
		<p>The AGENTS.md section does the rest — it teaches the LLM which aliases exist, how to run tests per language, and which env vars to set.</p>
	</div>
